...
void (*tabela_estados[]={})(stand_by, preenchendo, molho, lavagem, enxague, esvaziar);
...
int main(void)
{
  ...
  while (1)
  {
	...
	switch(state){
		case 0:
			if(HAL_GPIO_ReadPin(GPIOA, ON_Pin)==0)
			{
				state = 0;
			}
			else if ((HAL_GPIO_ReadPin(GPIOA, ON_Pin)==1 && 
					HAL_GPIO_ReadPin(GPIOA, SP_Pin)==1 ||
					HAL_GPIO_ReadPin(GPIOA, AV_Pin)==1)
			{
				state = 1;
			}
			break;
		case 1:
			if(HAL_GPIO_ReadPin(GPIOA, SNC_Pin)==0)
			{
				state = 1;
			}
			else
			{
				// criar botao de avanço
				state = 2;
			}
			break;
		case 2:		
			if(HAL_GPIO_ReadPin(GPIOA, AV_Pin)==1)
			{
				state = 3;
			}
			else
